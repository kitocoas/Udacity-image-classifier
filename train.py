...
parser.add_argument ('--save_dir',     help = 'Provide saving directory. (Optional)',              type = str  )
parser.add_argument ('--arch',         help = 'Vgg13 or Alexnet, Alexnet is default',              type = str  )
parser.add_argument ('--lrn',          help = 'Learning rate, default: lr = 0.002',                type = float)
parser.add_argument ('--hidden_units', help = 'Hidden units in the Classifier. Default = 2000',    type = int  )
...
print('Runining program in ',device)    
    
#data loading
if data_dir: 
    # Defining transforms for data sets
    ...
    # Loading datasets with images
    train_image_datasets = datasets.ImageFolder (train_dir, transform = train_data_transforms)
    valid_image_datasets = datasets.ImageFolder (valid_dir, transform = valid_data_transforms)
    test_image_datasets = datasets.ImageFolder (test_dir, transform = test_data_transforms)

    # Defining dataloader with the datasets
...
def load_model (arch, hidden_units):
    if arch == 'vgg13':
        ...
        else: #if hidden units set to default
            classifier = nn.Sequential  (OrderedDict ([
                        ('fc1', nn.Linear (25088, 4096)),
                        ('relu1', nn.ReLU ()),
                        ('dropout1', nn.Dropout (p = 0.3)),
        ...
        if hidden_units: #in case hidden_units were given
            classifier = nn.Sequential  (OrderedDict ([
                            ...
                            ('fc2', nn.Linear (3000, hidden_units)),
                            ('relu2', nn.ReLU ()),
                            ('dropout2', nn.Dropout (p = 0.3)),
                            ('fc3', nn.Linear (hidden_units, 102)),
                            ('output', nn.LogSoftmax (dim =1))
